...
void init(      map<string,TH1F> &h, 
    ...
    //~ // monte carlo backgrounds mit muonen
    tname = "template_bkg_num_MC";
    t[tname] = new TTree(tname.c_str(), tname.c_str());
    ...
    //~ // true monte carlo backgrounds (alles was nicht ee aus Z decay ist)
    tname = "template_bkg_num_MC_true";
    t[tname] = new TTree(tname.c_str(), tname.c_str());
    ...
    tname = "template_bkg_den_MC_true";
    t[tname] = new TTree(tname.c_str(), tname.c_str());
...
int f_my_singleelectron_MC_triggermatching(     map<string,TH1F> &h, 
    ...
    Float_t selW = 1.;
    //selW = *mc_weight * *pu_weight;// for now, no pu weight
    ...
    string triggerNames[] = {   "", //0
                                ...
                                "HLT_Ele27_eta2p1_WPLoose_Gsf_v2" // 9 singleElectron
...
*/
    ...
    while(reader.Next()){
        ...
        selW = Float_t(*mc_weight) * (*pu_weight);
        
        //cout << "*mc_weight = " << Float_t(*mc_weight) << endl;
...
*/        
        ...
        for(auto& tr: triggerObjects){
            ...
            if( std::find(  tr.pathNamesAll.begin(),
                                tr.pathNamesAll.end(),
                                triggerNames[9]) != tr.pathNamesAll.end()){
                ...
                if(selPhotons.size() >= 2){
                    ...
                    if(vp_tag.size() > 0){ // only do the calculations if at least one
                        ...
                        if(selPhotons.size() > 0){ // == 0 never happens here anymore!
                            ...
                            if(selProbe.hasPixelSeed){
                                nCounts[68]=0;
                                for(auto& tr: vtr4){ // loop trigger objects
...
int f_create_signalTemplate(                    map<string,TH1F> &h, 
    ...
    vector<tree::Photon*>   selPhotons,
                            selObjects, // for true background
                            vp_tag,
                            vp_probe;
...
int f_create_backgroundTemplate_MC(             map<string,TH1F> &h, 
                                                map<string,TH2F> &h2,
                                                map<string, Double_t*> &tb,
                                                string dataset,
                                                int *nCounts,
    ...
    cout << "f_create_backgroundTemplate_MC()" << endl;
    cout << dataset << endl;
    ...
    string triggerLabels[] = {  "", //0
                                ...
                                "hltEle27WPLooseGsfTrackIsoFilter"  // 9 singleElectron
                            };
...
*/
    ...
    while(reader.Next()){
        ...
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // estimate the background with a muon tag
        ...
        vp_tag_Photon.clear();
        vp_probe_Photon.clear();
        
        tagged = false;
        probed = false;
        
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        // now estimate the "real" background on the z peak;
        // use the selection of the signal template and then use every tag and probe pair
        // where tag and probe don't match to a generated electron
        
        
        // first, select a tag and a probe pair in the same way as in data
        if(*SingleElectron_4){
            ...
            bool    genTag   = true, // set to true if the selected  tag  corresponds to a generated electron
                    genProbe = true; //                             probe 
            ...
            if(selPhotons.size() >= 2){
                ...
                if(vp_tag_Photon.size() > 0){ // only do the calculations if at least one
                    ...
                    selTag_Photon = *(vp_tag_Photon[tc]);         // copy the chosen tag object to selTag;
                    vp_tag_Photon.erase(vp_tag_Photon.begin()+tc);// erase the chosen one from tag vector;
                    //cout << "tagchoice: " << tc << "\t" << selTag.p.Pt() << endl;
                    
                    // push the remaining tags that have not 
                    // been chosen back to selPhotons.
                    for(auto& tag: vp_tag_Photon){
                        selPhotons.push_back(tag);
                    }
                    ...
                    if(selPhotons.size() > 0){ // == 0 never happens here anymore!
                        ...
                        pc = rg->Integer(selPhotons.size());
                        selProbe_Photon = *(selPhotons[pc]);
                        //selPhotons.erase(selPhotons.begin()+tc); // not nescessary
                        //cout << "probechoice: " << pc << "\t" << selProbe.p.Pt() << endl;
                        
                        // HERE: one tag and one probe have been selected;
                        hname = "delR_selTag_selProbe";
                        h[hname].Fill(selTag.p.DeltaR(selProbe_Photon.p));
                        h2name = "plane_PhiEta_selTag_selProbe";
                        h2[h2name].Fill(    fabs(selTag_Photon.p.Phi()-selProbe_Photon.p.Phi()),
                            ...
                            if(fabs(interGen.pdgId) == 23){ // Z boson
                                for(auto& d: interGen.daughters){
                                    if(abs(d.pdgId) == 11){ // electrons/positrons
                                        ...
                                        // check tag
                                        delR = d.p.DeltaR(selTag_Photon.p);
                                        delPt = fabs(d.p.Pt()-selTag_Photon.p.Pt())/d.p.Pt();
                                        relPt = selTag_Photon.p.Pt() / d.p.Pt();
                                        ...
                                        // check probe
                                        delR = d.p.DeltaR(selProbe_Photon.p);
                                        delPt = fabs(d.p.Pt()-selProbe_Photon.p.Pt())/d.p.Pt();
                                        relPt = selProbe_Photon.p.Pt() / d.p.Pt();
                        ...
                        // if tag and probe both are not corresponding to a generated electron,
                        // fill the background
                        if(!genTag && !genProbe){
                            ...
                            // calculate lorentzvector from both:
                            v = selTag_Photon.p + selProbe_Photon.p;
                            E = selTag_Photon.p.Mag() + selProbe_Photon.p.Mag();
                            lv.SetVect(v);
                            lv.SetE(E);
                            ...
                            // to each other.
                            // Require this for tag and probe:  
                            if( selTag_Photon.p.DeltaR(selProbe_Photon.p) > 0.3
                                ){
                                    //cout << endl;
                                    //cout << "singleElectron_4!" << endl;
                                    ...
                                    *tb[bname] = lv.M();
                                    bname = "pt";
                                    *tb[bname] = selProbe_Photon.p.Pt();
                                    bname = "nvtx";
                                    *tb[bname] = *nGoodVertices;
                                    bname = "eta";
                                    *tb[bname] = fabs(selProbe_Photon.p.Eta());
                                    bname = "ntrk";
                                    *tb[bname] = *nTracksPV;
                                    t[tname]->Fill();
                                    bname = "w"; //weight
                                    *tb[bname] = selW;
                                    ...
                                    if(!selProbe_Photon.hasPixelSeed){
                            ...
                            if(selProbe.hasPixelSeed){
                                for(auto& tr: vtr4){
                                    delR = tr->p.DeltaR(selProbe_Photon.p);
                                    relPt = selProbe_Photon.p.Pt()/tr->p.Pt();
                                    if( delR < 0.1 &&
                                        relPt > 0.8 &&
                                        relPt < 1.4){
                                            ...
                                            /// is probe could also be tag
                                            /// add again to denumerator again
                                            
                                            //the two electrons should be back to back.
                                            // Require this for tag and probe:  
                                            if( selTag.p.DeltaR(selProbe_Photon.p) > 0.3
                                                ){
                                                    // fill denominator
                                                    tname = "template_bkg_den_MC_true";
                                                    bname = "m";
...
int f_closure(                                  map<string,TH1F> &h, 
                                                map<string,TH2F> &h2,
                                                map<string, Double_t*> &tb,
                                                string dataset,
                                                int *nCounts,
...
*/
    ...
    while(reader.Next()){
        ...
        //    if(fabs(interGen.pdgId) == 23){ // Z boson
                ...
                // preselect photon like and electorn like objects within the barrel
                //~ for(auto& p: photons){
                    //~ 
                    //~ if(fabs(p.p.Eta()) < 1.4442){
                        //~ if( p.hasPixelSeed) selPhotonsE.push_back(&p);
                        //~ if(!p.hasPixelSeed) selPhotons.push_back(&p);
                        //~ 
                    //~ }
                    //~ 
                //~ }
                ...
                // electron-like photon objects
                //~ if(selPhotonsE.size() > 0){
                    //~ 
                    //~ if( selPhotonsE[0]->p.Pt() > 50.
                        //~ ){
                            //~ hname = "clos_gen_e";
                            //~ h[hname].Fill(selPhotonsE[0]->p.Pt());
                    //~ }
                //~ }
                
                
                // photon like photon objects
                //if(selPhotons.size() > 0){
                    for(auto& d: genParticles){
                        if(abs(d.pdgId) == 11){ // electrons/positrons
                            v += d.p;
                            E += d.p.Mag();
                            ...
                            //~ delR = d.p.DeltaR(selPhotons[0]->p);
                            //~ relPt= selPhotons[0]->p.Pt() / d.p.Pt();
                            //~ 
                            //~ if( selPhotons[0]->p.Pt() > 40. &&
                                //~ delR < 0.1 &&
                                //~ relPt > 0.8 &&
                                //~ relPt < 1.2
                                //~ ){
                                    //~ 
                                    //~ hname = "clos_gen_g";
                                    //~ h[hname].Fill(selPhotons[0]->p.Pt());
                                        //~ 
                            //~ }
                ...
                //~ for(auto& d: genParticles){
                    //~ if(abs(d.pdgId) == 11){ // electrons/positrons
                        ...
                        //~ E += d.p.Mag();
                        //~ 
                        //~ if(!photons[0].hasPixelSeed && photons[0].p.Pt>40){
                            //~ 
                            //~ delR = d.p.DeltaR(photons[0].p);
                            //~ relPt= photons[0].p.Pt() / d.p.Pt();
                            //~ 
                            //~ 
                        //~ }
                        //~ 
