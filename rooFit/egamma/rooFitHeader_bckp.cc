...
{   // parametrized fit model(s)
    ...
    Double_t f = 0.9;       // signal fraction
    
    Double_t m_z = 91.1876; // Z mass in GeV
    Double_t w_z = 2.495;   // Z decay width in GeV
    
    // initialize signal and background fraction
    RooRealVar *nbkg = new RooRealVar("nbkg", "Background events", (1-f)*Ntot);//, 0.01*Ntot, 0.2*Ntot);
    RooRealVar *nsig = new RooRealVar("nsig", "Signal events", f*Ntot);//, 0.8*Ntot, Ntot);
    
    //fractions for nominator/denumerator
    if(fraction.find("den") != string::npos){
        cout << "Model for denumerator!!!!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << endl;
        f = 0.978;
...
void templateKernelUnbinnedGlobal(string signal, string background, string dataset){
    
    cout << "rooFitHeader.templateKernelUnbinnedGlobal()" << endl;
    ...
    TFile *fResults = new TFile("results_global1.root", "update");
    
    string subfolder = "withSigConvFFT";
    ...
    for(int i=0; i<Nts; i++){
        ...
        subfolder = "withSigConvFFT";
        tf[ts[i]]->buildPdf("hist", "fit");
        tf[ts[i]]->drawRaw(fResults, subfolder);
        tf[ts[i]]->drawFrame(fResults, subfolder);
        tf[ts[i]]->save(fResults, subfolder);
        
        // no convolution
        subfolder = "noSigConvFFT";
...
void templateKernelUnbinned(string signal, string background, string dataset, string mcset){
    ...
    rdsMc[mcname] = new RooDataSet( mcname.c_str(),
                                    ...
                                    RooArgList(*m, *w), 
                                    Import(*t[mcname]));
    // now use the weight-branch (w) to make roofit interpret them as a weight for the other branches
    mcname = "mc_num";
    ...
    mcname = "mc_num";
    thMc[mcname] = (TH1F*) rdsMc[mcname]->createHistogram(m->GetName(), 60);
    mcname = "mc_den";
    thMc[mcname] = (TH1F*) rdsMc[mcname]->createHistogram(m->GetName(), 60);
    
    
    //*(rdsData->createHistogram(m->GetName(), NbinsFit))
    ...
    map<string, RooAbsData*> rdsPt;     // pt
    map<string, RooAbsData*> rdsNtrk;   // ntrk
    map<string, RooAbsData*> rdsPtNtrk; // pt and ntrk
    map<string, RooAbsData*> rdsNvtx;   // nvtx
    map<string, RooAbsData*> rdsEta;    // Eta
    ...
    for(int i=0; i<Nts; i++){
        ...
        parametrizedModel = getModelPdf(*m, tf[ts[i]]->getNtot(), "modelGaus", ts[i]);
    ...
    //~ for(int i=0; i<Nts; i++){
        ...
        //~ tf[ts[i]]->addData(thMc["mc_"+ts[i]], t["template_sig"], t["template_bkg_"+ts[i]]);
        //~ 
        //~ tf[ts[i]]->setRhoBkg(1.);          // set rho to 1 (smoothness prefered over detail)
        //~ tf[ts[i]]->setUnbinnedFit(kFALSE); // binned fit
        //~ 
    ...
    //~ // now bin with respect to the variables ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    //for(int i=fitFraction; i<=fitFraction; i++){
        ...
        //for(int bin=fitPtBin ; bin <= fitPtBin; bin++){
            ...
            fSubfolder = unique+"pt/mengleiBins";
...
void readFitResults(string dropbox){
    ...
    //for(int i=fitFraction; i<=fitFraction; i++){
        for(int bin=0; bin < (Npt-1); bin++){
            ...
            rPlot->Draw();
            tPlot->SaveAs((dropbox+"thesis/ma_SusyMeeting/2016_06_16/plots/"+var+"/"+sName+".png").c_str());
            tPlot->Close();
    ...
    //~ c[var]->cd();
    //~ tGraph[var]->Draw("ap");
    //~ tGraph[var]->Write(("tGraph_"+var).c_str(), TObject::kOverwrite);
    //~ c[var]->Write(("canvas_"+var).c_str(), TObject::kOverwrite);
    //~ c[var]->SaveAs((dropbox+"thesis/ma_SusyMeeting/2016_06_16/fakerate/"+var+".png").c_str());
    //~ 
    //~ var = "nvtx";
    //~ c[var] = new TCanvas(("c_"+var).c_str(), ("c_"+var).c_str(), 700, 700);
    //~ c[var]->cd();
